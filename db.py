...
def seed_data():
    ...
    cursor.executemany(
        """INSERT OR REPLACE INTO "usuario" ("id","mac_addr","login","senha","role")
           VALUES (?,?,?,?,?)""",
        [
            (1, "BC:E9:2F:F9:AF:83", "admin", "admin123", "admin"),
...
def get_user_by_mac(mac):
    ...
    mac_query = mac.replace("0x", "").upper()
    mac_query = ":".join(mac_query[i:i+2] for i in range(0, len(mac_query), 2))
    

    query = "SELECT * FROM usuario WHERE mac_addr = ?"
    cursor.execute(query, (mac_query,))
